-- Supabase Storage Setup for Comment Photos
...
-- RLS Policies for comment-photos bucket
...
-- Create function to clean up orphaned photos (optional, for maintenance)
...
-- Create a function to validate photo URLs
...
-- Add constraint to comments table to validate photo URLs
ALTER TABLE comments
DROP CONSTRAINT IF EXISTS check_photo_url_format;

ALTER TABLE comments
ADD CONSTRAINT check_photo_url_format
CHECK (validate_photo_url(photo_url));
...
AND policyname LIKE '%comment photos%';
